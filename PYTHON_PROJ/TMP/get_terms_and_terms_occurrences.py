...
def main(args):
    ...
    output_pathname = os.path.join(output_folder, 'terms_occurrences.pkl')

    save_data(terms, output_pathname)

    df = pd.DataFrame(data=terms, index=[0])
    df = df.T
    output_pathname = os.path.join(output_folder, 'terms_occurrences.xlsx')
    df.to_excel(output_pathname)
